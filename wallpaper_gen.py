...
WALLPAPER_SOURCE = Path(__file__).parent / "source"
...
def load_tasks() -> dict:
    ...
    return {"daily": [], "weekly": [], "notes": "", "theme": "gradient", "resolution": "1920x1080"}
...
def render_date_panel(canvas: Image.Image, x: int, y: int, w: int):
    ...
    draw.text((x + w - 110, y + 38), time_str, font=font_time, fill=(*ACCENT, 240))
...
def generate_wallpaper(tasks_data: dict | None = None) -> Path:
    data = tasks_data or load_tasks()
    w, h = parse_resolution(data.get("resolution", "1920x1080"))
    theme = data.get("theme", "gradient")

    # img = build_background(theme, w, h)
